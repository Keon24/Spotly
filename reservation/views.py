...
class ReservationLotView(APIView):
    def post(self,request):
        ...
        return Response(
            {'message': 'Reservation Failed', "data": reserve_lot.data},
...
class ReservationDateView(APIView):
     def get (self,request):
         now = timezone.now()
         reserve_date = ReservationSerializer(ReservationLot.objects.filter(user=request.user, reserved_for__get=now,many=True))
...
class AvailableView(APIView):
    ...
    # Extract the lot IDs from these reservations
        reserved_lots = ReservationLot.objects.filter(reserve_date=reserve_date,soft_delete__isnull=True)    
